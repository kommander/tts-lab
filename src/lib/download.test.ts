...
test("downloads an asset and reports aggregate progress", async () => {
  directory = await mkdtemp(join(tmpdir(), "tts-lab-download-"))
  ...
  expect(await readFile(join(directory, "nested/model.bin"), "utf8")).toBe(content)
  expect(events.at(-1)).toBe(content.length)
})
...
test("promotes a complete partial without fetching it again", async () => {
  ...
  expect(await readFile(join(directory, "nested/model.bin"), "utf8")).toBe("complete")
})
